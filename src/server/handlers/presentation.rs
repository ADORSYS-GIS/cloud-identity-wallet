...
use crate::domain::models::presentation::{
    ...
    PresentationEngine, PresentationError, PresentationErrorCode, StartPresentationRequest,
    StartPresentationResponse, VerifierDirectPostResponse,
};
...
async fn handle_acceptance<S: SessionStore + Clone>(
    ...
    validate_selected_credentials(selections, session)?;
...
fn validate_selected_credentials(
...
async fn build_selected_credentials(
    ...
    for selection in selections {
        let cred_id = Uuid::parse_str(&selection.credential_id).map_err(|_| {
            PresentationError::invalid_credential_selection(format!(
                "'{}' is not a valid UUID",
