...
async fn list_credentials_does_not_return_other_tenants_credentials() {
    ...
    repo.upsert(make_credential(tenant_a), None).await.unwrap();
...
async fn list_credentials_filters_by_valid_status() {
    ...
    repo.upsert(revoked, None).await.unwrap();
    repo.upsert(make_credential(tenant_id), None).await.unwrap(); // active
...
async fn list_credentials_filters_by_valid_format() {
    ...
    repo.upsert(mdoc, None).await.unwrap();
    repo.upsert(make_credential(tenant_id), None).await.unwrap(); // dc+sd-jwt
...
async fn get_credential_returns_credential_for_owner() {
    ...
    repo.upsert(credential, None).await.unwrap();
...
async fn list_credentials_filters_by_issuer() {
    ...
    repo.upsert(other_issuer, None).await.unwrap();
    repo.upsert(make_credential(tenant_id), None).await.unwrap(); // issuer.example.com
...
async fn list_credentials_returns_expires_at_when_credential_has_expiry() {
    ...
    let id = expiring.id;
    repo.upsert(expiring, None).await.unwrap();
...
async fn list_credentials_credential_types_filter_excludes_unrelated_types() {
    ...
    repo.upsert(cred, None).await.unwrap();
