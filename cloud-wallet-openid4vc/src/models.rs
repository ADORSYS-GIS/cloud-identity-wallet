...
/// Lifecycle status of a stored [`Credential`] as tracked by the wallet.
...
/// A reference to an issuer-maintained status list used for revocation checks.
///
/// Stored in **plaintext** so the wallet can perform revocation checks without
/// first decrypting the credential payload.
///
/// Typically extracted from the `status` claim of an SD-JWT or W3C VC on
/// first storage and cached here for efficient look-ups.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StatusReference {
    /// The URL of the OAuth Status List or equivalent revocation endpoint.
    pub url: String,

    /// The index within the status list that corresponds to this credential.
    pub index: u64,
}
...
pub struct Credential {
    ...
    /// Optional issuer status-list reference for revocation checks.
    ///
    /// Stored in plaintext alongside the encrypted payload so revocation can
    /// be verified without decrypting the credential.
