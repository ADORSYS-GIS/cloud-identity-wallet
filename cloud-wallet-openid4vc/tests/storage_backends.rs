...
async fn test_repository_backend<R: CredentialRepository>(
    repository: &R,
    tenant_a: Uuid,
    tenant_b: Uuid,
) {
    let credential_a = common::sample_credential(tenant_a);
    let mut credential_b = common::sample_credential(tenant_b);
